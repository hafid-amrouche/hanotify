...
import json
from .serializers import OrderPreviewSerializer, OrderDetailsSerializer
...
def create_order(request):
    ...
    product_dict = {
        ...
        'title': product.title,
        'price': price,
        'shipping_home_cost': shipping_state_cost.cost_to_home if shipping_state_cost else None,
        'shipping_office_cost': shipping_state_cost.cost if shipping_state_cost else None,
        'id': product.id,
    ...
    order = Order.objects.create(
        ...
        shipping_city = City.objects.get(id= data.get('city_id'), state=shipping_state),
        shipping_address = data.get("shippig_address").strip(),
...
def update_order(request):
    try:
        ...
        order.shipping_city = City.objects.get(id= city, state=shipping_state)
...
def confirm_order(request): ## add this front end
    try:
        ...
        City.objects.get(id=city_id, state=shipping_state)
